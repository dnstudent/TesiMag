...
tag_same_series <- function(analysis) {
    analysis |> mutate(
        ...
        tag_sseries_ivi = (dataset_x == "ISAC" & dataset_y == "ISAC") & (
            (valid_days_inters >= 160L & ((f0 > 0.102 & distance < 1800) | strSym > 0.99 | distance < 150))
        ),
        ...
        tag_same_series = tag_sseries_avi | tag_sseries_avs | tag_sseries_ivs | tag_sseries_ivi | tag_sseries_svs | user_code_x == user_code_y,
        tag_mergeable = TRUE
    )
}
    ...
    tagged_analysis |> mutate(
        tag_same_series = tag_same_series & !(
            ((dataset_x == "ISAC" & dataset_y == "SCIA") &
                ...
                (((dataset_x == "ISAC" & network_x == "ISAC") | (dataset_y == "ISAC" & network_y == "ISAC")) & (user_code_x != user_code_y))
        )
    )
}
