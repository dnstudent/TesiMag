...
query_checkpoint_meta("full", "merged_corrected", conns$data) |>
  ...
  write_csv_arrow(fs::path(save_to, "metadata", "metadata.csv"), write_options = csv_write_options(quoting_style = "AllValid"))

open_dataset(fs::path("db", "extra", "merge_specs")) |>
  collect() |>
  assertr::assert(not_na, everything()) |>
  unite("from", from_dataset, from_sensor_key, sep = "/") |>
  write_csv_arrow(fs::path(save_to, "metadata", "merge_specs.csv"), write_options = csv_write_options(quoting_style = "AllValid"))
```
...
corrections_path <- fs::path(save_to, "metadata", "manual_corrections.csv")
...
corrections1 |>
  ...
  readr::write_csv(fs::path(save_to, "metadata", "manual_corrections.csv"), na = "")
```
...
original_metadata <- query_checkpoint_meta(datasets, "raw", conns$data) |>
  collect()
original_metadata |> readr::write_csv(fs::path(save_to, "metadata", "starting_metadata.csv"))
```
