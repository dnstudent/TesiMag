...
# if (fs::dir_exists(ds4merge_dir)) unlink(ds4merge_dir, recursive = TRUE)
# merged_ds$data |>
#   collect() |>
...
#     path <- fs::path(ds4merge_dir, str_glue("dataset={dataset}"), str_glue("sensor_key={sensor_key}"), str_glue("variable={variable}"))
...
merge_specs |>
  ...
  purrr::walk(~ {
    ...
    save_to_path <- fs::path(save_to_root, "data", str_glue("{variable_name[[variable]]}_{set}_{gkey}.csv"))
