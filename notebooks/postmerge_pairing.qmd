...
saved_to <- merge_same_series(path_from, fs::path("db", "tmp", "rawremerged"), new_dataset, tagged_analysis, metadata, data, 1, 2L * 365L, NULL, desc(dataset), desc(sensor_last), desc(network))
...
dps |> openxlsx::write.xlsx(file.path("db", "conv", "merged_corrected", "merge_duplicates.xlsx"), asTable = TRUE)
```
