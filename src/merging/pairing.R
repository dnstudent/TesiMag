...
pair_full_series <- function(x, lagged_match_list, by = c("id_x", "id_y", "variable")) {
    left <- x |>
        rename(id_x = station_id) |>
        # inner_join(lagged_match_list |> select(starts_with("id"), variable), by = c("id_x", "variable"))
        inner_join(lagged_match_list |> select(all_of(by)), by = c("id_x", "variable"), copy = TRUE)

    right <- x |>
        rename(id_y = station_id) |>
        inner_join(lagged_match_list |> select(all_of(by), offset_days), by = c("id_y", "variable"), copy = TRUE) |>
