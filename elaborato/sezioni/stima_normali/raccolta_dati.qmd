...
<!--- Serie buona: Lucca TOS/111, Levico TAA2/80 -->

```{r}
#| label: fig-rawseriesexample
#| fig-cap: "Serie di Levico (TN). Otto sequenze concorrono alla costituzione della serie."
#| fig-subcap:
#|  - "Posizione delle stazioni."
#|  - "Sequenze delle temperature massime giornaliere. La DPC/1775 è stata omessa perché troppo recente."
#| cache: false
#| fig-asp: 0.618
#| layout-ncol: 2
#| column: page
...
eight_series <- ds |>
    filter(dataset == "TAA2", series_key == 80L) |>
    collect()
eight_stats <- query_checkpoint_meta(c("TAA", "ISAC", "SCIA"), "raw", conns$data) |>
...
# Map
wdbbox <- sf::st_bbox(eight_stats |> sf::st_buffer(units::set_units(100, "m")))
...
data <- query_checkpoint_data(c("TAA", "ISAC", "SCIA"), "raw", conns$data) |>
...
#| fig-cap: "Grafo della serie di Levico (TN). La procedura di rilevazione dei *match* non è in grado di connettere tutte le stazioni, tuttavia le "
#| cache: false
#| fig-asp: 0.618

source("src/merging/combining.R")
levico <- openxlsx::read.xlsx("elaborato/sezioni/stima_normali/levico.xlsx", sheet = "Foglio1") |>
    as_tibble() |>
    filter(variable == 1)

graph_from_isedge(levico |> mutate(key_x = sensor_key_x, key_y = sensor_key_y), tag_same_series, FALSE) |> plot()
```

```{r}
#| label: tbl-compexample
#| tbl-cap: "Estratto del confronto di sequenze e metadati delle stazioni di Levico per le massime giornaliere."

library(knitr)
options(knitr.kable.NA = "")
levico |>
    mutate(dataset_x = if_else(dataset_x == "ISAC", network_x, dataset_x), dataset_y = if_else(dataset_y == "ISAC", network_y, dataset_y), sequence_x = str_c(dataset_x, sensor_key_x, sep = "/"), sequence_y = str_c(dataset_y, sensor_key_y, sep = "/")) |>
    select(starts_with("sequence"), distance, delH, f0, delT, maeT, balance, valid_days_inters) |>
    slice_head(n = 5L) |>
    kable(col.names = c("Sequenza 1", "Sequenza 2", "Distanza [m]", "∆ Quota [m]", "$\\mathrm{f}_0$", "$\\langle \\Delta T \\rangle$ [°C]", "$\\langle \\abs{\\Delta T} \\rangle$ [°C]", "b", "$\\mathrm{n_d}$"), digits = c(0, 0, 0, 0, 2, 2, 2, 2, 0))
```
